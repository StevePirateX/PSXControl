...
class PSXControlNetThread extends Thread {
	...
	void functionInactive() {
		...
		send("Qs89=" + CduMenu.qs89Menu + CduMenu.defaultLongRightMenu);
	}
					...
					try {
						if (message.charAt(0) == 'Q') {
							...
							try {
								...
								if (qCategory == 's') {
									...
									if (qIndex >= 4 && qIndex <= 61) {
										...
										MenuService.cbs[qIndex - 3][0] = "Qs" + qIndex;
										MenuService.cbs[qIndex - 3][1] = raw;
									}
									...
									if (qIndex == 121) {
										...
										MenuService.altitude = Integer.parseInt(pos[3]) / 1000;
										MenuSituations.verticalSpeed = Situations
												.getVerticalSpeed(Integer.parseInt(pos[3]));
										MenuSituations.yawRate = Situations.getVerticalSpeed(MenuWeather.hdg);
									...
									if (qIndex == 123) {
										...
										if (!CduMenu.viewPSXTime.equals(hm.format(CduMenu.psxTime.getTime()))) {
											...
											if (PSXControl.active && CduMenu.activePage == CduMenu.PAGE_SITUATIONS
													&& !isPsxLoading) {
									...
									if (qIndex == (328 + PSXControl.psx.wxZone)) {
										String raw = message.substring(parseMark).trim();
										MenuWeather.weather = message.substring(parseMark).trim();
										PSXControl.psx.weatherArray = MenuWeather.weather.split(";");
									...
									if (qIndex >= 343 & qIndex <= 349) {
										...
										if (PSXControl.psx.wxZone == i + 1) {
											...
											if (MenuWeather.weatherMetars[i].substring(0, 1).equals("i")) {
												MenuWeather.weatherIcao = MenuWeather.weatherMetars[i].substring(1, 5);
											} else {
												MenuWeather.weatherIcao = MenuWeather.weatherMetars[i].substring(0, 4);
											}
									...
									if (qIndex == 392) {
										String raw = message.substring(parseMark).trim();
										MenuService.trans = Integer.parseInt(raw.substring(raw.indexOf(";") + 1,
									...
									if (qIndex == 438) {
										...
										for (int i = 0; i < 9; i++) {
											...
											if (i == 0) {
												...
												MenuSituations.fuel = 0;
												fuelOnBoard = 0;
											}
											fuelOnBoard += Integer.valueOf(tanks[i]);
										}

										MenuSituations.fuel = (float) (fuelOnBoard / 2.2046226218 / 10000);
										// System.out.println("Total Fuel (kgs): " + CduMenu.fuel);

										if (PSXControl.active && CduMenu.activePage == CduMenu.PAGE_SITUATIONS)
											CduMenu.updateMenu(0);
									}

									// EFIS PANEL SYNC (for QNH)
									if (qIndex == 448) {
										String raw = message.substring(parseMark).trim();
										MenuService.Qs448 = raw.split(";");
									}
									if (qIndex == 449) {
										String raw = message.substring(parseMark).trim();
										MenuService.Qs449 = raw.split(";");
									}

									if (qIndex == 483 && CduMenu.activePage == CduMenu.PAGE_WEATHER) {
										CduMenu.updateMenu(1);
									}
									...
									// CG
									if (qIndex == 122) {
										float raw = Float.parseFloat(message.substring(parseMark).trim()) / 10;
										// System.out.println(message);
										MenuSituations.cg = raw;
										// System.out.println(String.format("%.2f", CduMenu.trim));
										if (PSXControl.active && CduMenu.activePage == CduMenu.PAGE_SITUATIONS)
											CduMenu.updateMenu(0);
									}
									...
									if (qIndex == 129) {
										...
										if (activeMenu == 1 && !MenuSituations.didAddonCauseReload && PSXControl.active)
											CduMenu.updateMenu(CduMenu.PAGE_MAINMENU);
									}
							...
							try {
								...
								System.out.println("Connection OK. CLIENT ID: " + clientId);
							} catch (NumberFormatException nfe) {
								nfe.printStackTrace();
							}
							...
							viewMessages = false;
							if (!MenuSituations.didAddonCauseReload) {
								MenuSituations.situIVariables = PSXControl.psx.qiVariables.clone();
								MenuSituations.situHVariables = PSXControl.psx.qhVariables.clone();
								MenuSituations.situSVariables = PSXControl.psx.qsVariables.clone();
							}
							MenuSituations.didAddonCauseReload = false;
