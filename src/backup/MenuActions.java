...
public class MenuActions extends CduMenu {
	...
	public static void showMenu() {
		...
		int titleLength = titlMenu.length() - String.valueOf(page).length() - String.valueOf(totalPages).length() - 2;
		String titleBig = new String(new char[titleLength]).replace("\0", "+");
		String titleSmall = new String(new char[24 - titleLength]).replace("\0", "-");

		send("Qs76=" + titlMenu.substring(0, titleLength) + page + "/" + totalPages + "_" + titleBig + titleSmall);
		
		send("Qs77=" + qs77Menu + "____________GND_");
		motionFreeze((isGroundProxBypassed ? "<ACT>" : "_____") + "_PROX>");
		send("Qs79=" + qs79Menu + "__FLT_DECK_DOOR_");
		send("Qs80=" + qs80Menu + "________" + (isFltDoorLocked ? menuFltDeckDoor[0] : menuFltDeckDoor[1])
				+ ">");
		send("Qs81=" + qs81Menu + "_____INTERPHONE_");
		send("Qs82=" + qs82Menu + "<ACT>__" + (isInterphoneActive ? menuInterphone[0] : menuInterphone[1]) + ">");
